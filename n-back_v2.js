...
function endRoutineBegin(snapshot) {
  return async function () {
    ...
    //nCorr = ((psychoJS.experiment['key_resp.corr'].sum() / 5) * 100);
    //meanRT = (psychoJS.experiment['key_resp.rt'].mean() * 1000);
    ...
    nCorr = Math.round(nCorr/responses.length*100)
    // keep only trials with keystrokes
    //meanRT = rt_list.reduce((a, b) => a + b['key_resp.rt'], 0) / rt_list.length  // get mean
    rt_sum = 0
    nb_key = 0
    ...
    meanRT = Math.round(rt_sum/nb_key * 1000)  // get mean
